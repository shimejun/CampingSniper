...
execute if entity @e[tag=!team] run title @a subtitle [{"text":"Winner:","color":"gold","bold":true},{"text":"PURPLE TEAM","color":"dark_purple"}]
...
clear @a
tellraw @a [{"text":"Winner: ","color":"gold"},{"selector":"@a[team=purple]","color":"aqua"}]
scoreboard players add @a[team=purple] win_total 1
...
execute if score #teamtype id matches 2 run advancement grant @a[team=purple] only owgame:badges/team_duowin
execute if score #teamtype id matches 3 run advancement grant @a[team=purple] only owgame:badges/team_triowin
...
execute if score #teamtype id matches 6 as @a[team=purple,gamemode=adventure] at @s unless entity @a[team=purple,distance=0.1..] run advancement grant @s only owgame:badges/team_solowin_in6team
