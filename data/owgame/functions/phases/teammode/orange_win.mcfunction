...
execute if entity @e[tag=!team] run title @a subtitle [{"text":"Winner:","color":"gold","bold":true},{"text":"ORANGE TEAM","color":"gold"}]
...
clear @a
tellraw @a [{"text":"Winner: ","color":"gold"},{"selector":"@a[team=orange]","color":"aqua"}]
scoreboard players add @a[team=orange] win_total 1
...
execute if score #teamtype id matches 6 run advancement grant @a[team=orange] only owgame:badges/team_hexawin
execute unless entity @a[team=orange,gamemode=spectator] run advancement grant @s only owgame:badges/team_win
execute as @a[team=orange,gamemode=adventure] at @s unless entity @a[team=orange,gamemode=adventure,distance=1..] run advancement grant @s only owgame:badges/team_clutch
